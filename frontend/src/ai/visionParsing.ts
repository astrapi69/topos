...
function toRecognizedItem(entry: unknown): RecognizedItem | null {
  ...
  return {
    label,
    categoryPath:
      typeof record.category_path === "string" ? record.category_path : "",
    newCategoryHint:
      typeof record.new_category_hint === "string"
        ? record.new_category_hint
        : "",
    description:
...
export function parseItemsPayload(payload: unknown): RecognizedItem[] {
  let decoded: unknown =
    typeof payload === "string" ? decodeJsonText(payload) : payload;
  if (
