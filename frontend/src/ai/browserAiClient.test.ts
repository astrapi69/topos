...
function resolved(
  overrides: Partial<ResolvedLocalProvider> = {},
...
describe("googleResponseSchema", () => {
  it("uppercases types and drops additionalProperties and bounds", () => {
    ...
    const confidence = (
      entry.properties as Record<string, Record<string, unknown>>
    ).confidence;
...
describe("testAiConnectionDirect", () => {
  ...
  it("maps 401 to auth_error and 500 to provider_error", async () => {
    ...
    expect(
      (await testAiConnectionDirect({ provider: "openai", apiKey: "k" }))
        .errorCode,
    ).toBe("auth_error");
...
describe("recognizePhotoDirect", () => {
  it("calls anthropic with forced tool use and parses the tool input", async () => {
    ...
    expect(payload.messages[0].content[1].text).toContain(
      "finance/bank, tools",
    );
  ...
  it("retries openai-compatible servers once without response_format on 400", async () => {
    fetchMock
      .mockResolvedValueOnce(
        jsonResponse(400, { error: { message: "no json_schema" } }),
      )
      .mockResolvedValueOnce(
        jsonResponse(200, {
          choices: [
  ...
  it("maps 429 to a rate-limit ApiError and transport failure to 502", async () => {
    ...
    const failure = await recognizePhotoDirect(
      resolved(),
      RECOGNIZE_OPTIONS,
    ).catch((recognizeError) => recognizeError);
