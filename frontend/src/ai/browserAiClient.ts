...
const ITEM_PROPERTIES: JsonRecord = {
  ...
  new_category_hint: {
    type: "string",
    description:
      "english-kebab-case proposal when no existing category fits, else empty.",
  },
  description: {
    type: "string",
    description: "Brief German description of the item.",
  },
...
export function googleResponseSchema(
  node: JsonRecord = ITEMS_JSON_SCHEMA,
): JsonRecord {
...
async function postJson(
  ...
  try {
    ...
    const reason =
      ...
      (transportError as Error).name === "AbortError"
