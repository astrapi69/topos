...
function pickStringMap(
  value: unknown,
): Partial<Record<ToposProviderId, string>> {
...
export function hasVault(): boolean {
  ...
  return Boolean(
    raw && looksLikeVaultEnvelope(raw, { format: TOPOS_VAULT_FORMAT }),
  );
}
...
export function resolveActiveProvider(): ResolvedLocalProvider | null {
  if (!isEnabled() || !isUnlocked() || !sessionKeys || !sessionSettings)
    return null;
  ...
  return {
    providerId: descriptor.id as ToposProviderId,
    apiKey,
    baseUrl,
    model,
  };
