...
function markdownToHtml(md: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (headingMatch) {
      htmlLines.push(
        `<h${headingMatch[1].length}>${headingMatch[2]}</h${headingMatch[1].length}>`,
      );
      continue;
    }
    ...
    if (imgMatch) {
      ...
      if (captionMatch) {
        htmlLines.push(
          `<figure><img src="${imgMatch[2]}" alt="${imgMatch[1]}" /><figcaption>${captionMatch[1]}</figcaption></figure>`,
        );
        i++;
      } else {
...
describe("markdownToHtml", () => {
  ...
  it("converts image", () => {
    expect(markdownToHtml("![alt](src.png)")).toBe(
