...
export type SecretSourceKind =
  "env" | "secrets_yaml" | "app_yaml" | "auto_generated";
...
export const api = {
  ...
  items: {
    ...
    bulkCreate: (bulkItems: BulkItemCreate[]) =>
      request<BulkItemsResult>("/items/bulk", {
        method: "POST",
  ...
  ai: {
    ...
    recognize: (
      ...
      return request<VisionResult>("/ai/vision", {
        method: "POST",
        rawBody: fd,
      });
    },
  ...
  photos: {
    ...
    remove: (containerId: number, photoId: number) =>
      request<void>(`/containers/${containerId}/photos/${photoId}`, {
        method: "DELETE",
      }),
  },
  backup: {
  ...
  settings: {
    ...
    deleteAiKey: (provider: string) =>
      request<AiKeyStatus>(`/settings/ai/keys/${provider}`, {
        method: "DELETE",
      }),
  },
  health: () =>
