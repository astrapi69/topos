...
export default function ContainerLabelsDialog({
  ...
  const allSelected =
    containers.length > 0 && selected.size === containers.length;
  ...
  async function handlePrint() {
    ...
    try {
      await printContainerLabels(chosen, {
        documentTitle: t(
          "topos.page.containers.labels_doc_title",
          "Topos Etiketten",
        ),
      ...
      notify.error(
        errorMessage(
          err,
          t(
            "topos.page.containers.labels_failed",
            "Etiketten konnten nicht erstellt werden",
          ),
        ),
        err,
      );
