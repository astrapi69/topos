...
export default function BackendUrlSettings() {
  ...
  async function probe(target: string): Promise<boolean> {
    ...
    try {
      const res = await fetch(`${apiBase(target)}/health`, {
  ...
  async function handleTest() {
    ...
    try {
      ...
      if (!ok) {
        notify.error(
          t(
            "topos.page.settings.backend_unreachable",
      ...
      notify.success(
        t(
          "topos.page.settings.backend_connected",
          "Verbunden. Daten werden vom Backend geladen.",
        ),
      );
    } catch (e) {
      notify.error(
  ...
  return (
    <section
      ...
      data-testid="backend-url-section"
    >
      ...
      <div className="flex flex-col gap-2 max-w-md">
        ...
        <p className={`${muted} text-sm`} data-testid="backend-url-current">
          {saved
            ? t(
                "topos.page.settings.backend_current",
                "Aktuell verbunden: {url}",
              ).replace("{url}", saved)
            : t(
                "topos.page.settings.backend_same_origin",
                "Aktuell: gleicher Host (/api)",
              )}
        </p>
      </div>
    </section>
