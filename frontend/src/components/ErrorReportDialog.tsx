  ...
  useEffect(() => {
    function handleOpen(event: Event) {
      ...
      setState({
        ...
        apiError:
          detail.apiError instanceof ApiError ? detail.apiError : undefined,
      });
    }
    window.addEventListener("topos:open-error-report", handleOpen);
    return () =>
      window.removeEventListener("topos:open-error-report", handleOpen);
  }, []);
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <label htmlFor="error-report-steps" className="text-sm font-medium">
            {t(
              "topos.error_report.steps_label",
          ...
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            ...
            />
            {t(
              "topos.error_report.include_env",
              "Umgebungsinfo (Version, Browser, OS)",
            )}
          </label>
