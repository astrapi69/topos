...
export default function ContainerPhotos({containerId}: {containerId: number}) {
    const {t} = useI18n();
    ...
    async function onFilesPicked(event: React.ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            await refresh();
            notify.success(
                t("topos.page.container_detail.photos_added", "{count} Foto(s) hinzugefügt").replace(
                    "{count}",
                    String(files.length),
                ),
            );
            ...
            notify.error(
                errorMessage(
                    err,
                    t("topos.page.container_detail.photos_add_failed", "Foto konnte nicht hinzugefügt werden"),
                ),
                err,
            );
        } finally {
    ...
    async function handleDelete(photoId: number) {
        const ok = await confirm(
            ...
            t("topos.page.container_detail.photos_delete_message", "Dieses Foto wird dauerhaft gelöscht."),
            "danger",
        );
        if (!ok) return;
        try {
            ...
            notify.error(
                errorMessage(
                    err,
                    t("topos.page.container_detail.photos_delete_failed", "Foto konnte nicht gelöscht werden"),
                ),
                err,
            );
        }
    ...
    return (
        <section style={{marginTop: "1.5rem"}} data-testid="container-photos">
            ...
            {items.length === 0 ? (
                ...
                >
                    {items.map((photo, i) => (
                        <li key={photo.id} className="relative" data-testid={`container-photo-${photo.id}`}>
                            <button
                                type="button"
                                onClick={() => setLightbox(i)}
                                className="block w-full"
                                aria-label={t("topos.page.container_detail.photos_open", "Foto öffnen")}
                            >
                                <img
                                    src={photo.thumbSrc}
                                    alt=""
