...
vi.mock("../photos", () => ({
  ...
  useContainerPhotos: () => ({
    items: [
      { id: 1, thumbSrc: "thumb-1", fullSrc: "full-1", mime: "image/jpeg" },
    ],
    loading: false,
...
describe("ContainerPhotos", () => {
  ...
  it("uploads picked files via addPhoto", async () => {
    ...
    fireEvent.change(screen.getByTestId("container-photos-input"), {
      target: { files: [file] },
    });
  ...
  it("opens the lightbox on thumbnail click", () => {
    render(<ContainerPhotos containerId={5} />);
    fireEvent.click(
      screen.getByTestId("container-photo-1").querySelector("button")!,
    );
