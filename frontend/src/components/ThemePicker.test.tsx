...
describe("ThemePicker", () => {
    ...
    it("marks the active theme's radio as checked (light default)", () => {
        render(<ThemePicker />);
        const lightRadio = within(screen.getByTestId("theme-option-light")).getByRole("radio");
        expect(lightRadio).toBeChecked();
    });

    it("selecting a theme applies data-app-theme + data-theme family", () => {
        render(<ThemePicker />);
        const hcRadio = within(screen.getByTestId("theme-option-high-contrast")).getByRole("radio");
        fireEvent.click(hcRadio);
        expect(document.documentElement.getAttribute("data-app-theme")).toBe("high-contrast");
