...
describe("AboutSection", () => {
  it("renders the version card with the build version", () => {
    ...
    // Manual "check for updates" control (pwa-update-react).
    expect(screen.getByTestId("about-update-check")).toBeInTheDocument();
    expect(screen.getByTestId("about-share-app")).toBeInTheDocument();
    // __APP_VERSION__ is injected by Vite (package.json version).
    expect(screen.getByTestId("about-app-version").textContent).toMatch(
      /\d+\.\d+\.\d+/,
    );
  });
  ...
  it("offers the three donation channels with Liberapay preferred", () => {
    ...
    expect(
      screen.getByTestId("about-donation-github-sponsors-link"),
    ).toHaveAttribute("href", "https://github.com/sponsors/astrapi69");
