...
describe("QrCodeModal", () => {
    it("renders the QR image and the encoded URL", async () => {
        render(<QrCodeModal url="https://example.test/topos/containers/5" title="Share" onClose={vi.fn()} />);
    ...
    it("copies the URL and reports it", async () => {
        ...
        render(<QrCodeModal url="https://example.test/x" title="Share" onClose={vi.fn()} onCopied={onCopied} />);
        fireEvent.click(screen.getByTestId("qr-code-modal-copy"));
        await waitFor(() => expect(writeText).toHaveBeenCalledWith("https://example.test/x"));
        expect(onCopied).toHaveBeenCalled();
    });

    it("closes on the X button and on Escape", () => {
        const onClose = vi.fn();
        render(<QrCodeModal url="https://example.test/x" title="Share" onClose={onClose} />);
