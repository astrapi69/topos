...
const REPORT = {
    orphanedItems: [
        {id: 42, content: "Alte Vertraege", categoryPath: "ghost/gone", containerId: 7},
        {id: 43, content: "Belege", categoryPath: "ghost/gone/deeper", containerId: 7},
    ],
    count: 2,
};
...
describe("OrphanPathsSection", () => {
    ...
    it("stays hidden when the report is unreachable (PWA mode)", async () => {
        ...
        expect(screen.queryByTestId("orphan-paths-section")).not.toBeInTheDocument();
    });
    ...
    it("shows the empty state when nothing is orphaned", async () => {
        ...
        expect(screen.queryByTestId("orphan-paths-remove-all")).not.toBeInTheDocument();
    });
