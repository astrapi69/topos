...
export default function ContainerDetail() {
  ...
  async function handleSaveContainer(e: React.FormEvent) {
    ...
    if (!edit.label.trim()) {
      notify.warning(
        t(
          "topos.page.containers.label_required",
    ...
    try {
      ...
      await refresh();
      notify.success(
        t("topos.toast.container_updated", "Container aktualisiert"),
      );
      setEditing(false);
    } catch (err) {
      notify.error(
  ...
  async function handleDeleteContainer() {
    ...
    let message = t(
      "topos.confirm.delete_container_message",
      'Der Container "{label}" wird dauerhaft gelöscht.',
    ).replace("{label}", container.label);
    if (itemCount > 0) {
      message +=
        " " +
        t(
    ...
    try {
      ...
      notify.error(
        errorMessage(
          e,
          t(
            "topos.toast.container_delete_failed",
            "Container konnte nicht gelöscht werden",
          ),
        ),
        e,
      );
    ...
    try {
      ...
      notify.error(
        errorMessage(
          e,
          t(
            "topos.toast.item_delete_failed",
    ...
    <>
      ...
      <main className="p-4 sm:p-6">
        <Link
          to="/containers"
          className={link}
          data-testid="container-detail-back"
        >
          ...
          <h1 data-testid="container-detail-title">
            {container?.label ??
              (loading
                ...
                : t(
                    "topos.page.container_detail.missing",
          ...
          {container && !editing && (
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
              ...
              <button
                type="button"
                className={btn}
                data-testid="container-detail-edit"
                onClick={openEdit}
              >
          ...
          >
            ...
            <FormField label={t("topos.container.owner", "Eigentümer")}>
              <select
                ...
                value={edit.owner}
                onChange={(e) =>
                  setEdit({ ...edit, owner: e.target.value as Owner })
                }
              >
                <option value="self">{t("topos.owner.self", "Ich")}</option>
                <option value="parents">
                  {t("topos.owner.parents", "Eltern")}
                </option>
                <option value="shared">
                  {t("topos.owner.shared", "Geteilt")}
                </option>
              </select>
            </FormField>
            ...
            <FormField label={t("topos.container.size_group", "Größengruppe")}>
              <input
                ...
                onChange={(e) =>
                  setEdit({ ...edit, sizeGroup: e.target.value })
                }
              />
            </FormField>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              <button
                ...
                data-testid="container-edit-save"
                disabled={saving}
              >
                {saving
              ...
              <button
                type="button"
                className={btn}
                data-testid="container-edit-cancel"
        ...
        {container && !editing && (
          ...
          >
            ...
            <dd>
              {t(`topos.container.type.${container.type}`, container.type)}
            </dd>
        ...
        <section>
          ...
          >
            ...
            <button
              ...
              onClick={() => navigate(`/items/new?container_id=${containerId}`)}
            >
          ...
          <div data-testid="container-detail-items" className="mt-2">
              ...
              return (
                ...
                >
                  <div>
                    ...
                    >
                      {t(
                        "topos.page.container_detail.item_actions",
                        "Aktionen",
                      )}
                      : {itemActions.length}
                  ...
                  <div>
                    <span className={cellLabel}>
                      {t("topos.item.priority", "Priorität")}:{" "}
                    </span>
                    {t(`topos.priority.${item.priority}`, item.priority)}
                  </div>
                  <div>
                    <span className={cellLabel}>
                      {t("topos.item.category", "Kategorie")}:{" "}
                    </span>
                    {item.categoryPath ?? ""}
                  </div>
            ...
            {items.data.length === 0 && !items.loading && (
              <div
                data-testid="container-detail-empty"
                className={`${muted} p-4`}
              >
                {t(
      ...
      {showQr && container && containerId !== null && (
        <QrCodeModal
          ...
          title={t(
            "topos.page.container_detail.share_qr_title",
            "Container teilen",
          )}
