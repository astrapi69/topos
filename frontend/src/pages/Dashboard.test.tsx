...
describe("Dashboard", () => {
  ...
  it("searches the cached data via MiniSearch and renders a hit", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("stat-containers")).toHaveTextContent("1"),
    );

    fireEvent.change(screen.getByTestId("dashboard-search-input"), {
      target: { value: "Citibank" },
    });

    await waitFor(
      () =>
        expect(
          screen.getByTestId("search-hit-container-3"),
        ).toBeInTheDocument(),
      { timeout: 2000 },
    );
  ...
  it("shows an empty state when nothing matches", async () => {
    ...
    await waitFor(
      () =>
        expect(
          screen.getByTestId("dashboard-search-empty"),
        ).toBeInTheDocument(),
      {
        timeout: 2000,
      },
