...
export default function PhotoIntake() {
  ...
  const selectedContainer = containers.find(
    (row) => String(row.id) === containerId,
  );
  const committableRows = staged.filter(
    (row) => row.selected && row.label.trim() !== "",
  );
  ...
  async function handleRecognize() {
    ...
    if (!privacyAccepted) {
      const accepted = await confirm(
        t(
          "topos.page.photo_intake.privacy_title",
          "Foto an AI-Provider senden?",
        ),
        t(
  ...
  function toBulkPayload(row: StagedRow): BulkItemCreate {
    const wantsNewCategory =
      row.createNewCategory && row.newCategoryHint.trim() !== "";
    return {
      ...
      categoryPath:
        !wantsNewCategory && row.categoryPath ? row.categoryPath : null,
  ...
  async function handleCommit() {
    ...
    try {
      ...
      if (bulkResult.errors.length === 0) {
        notify.success(
          t(
            "topos.page.photo_intake.committed",
            "{count} Items übernommen",
          ).replace("{count}", String(bulkResult.created.length)),
        );
      ...
      const failedIndices = new Set(
        bulkResult.errors.map((error) => error.index),
      );
      ...
      setStaged((previous) =>
        previous.filter((row) => failedKeys.has(row.key)),
      );
      notify.error(
        t(
      ...
      notify.error(
        errorMessage(
          err,
          t("topos.toast.photo_commit_failed", "Übernahme fehlgeschlagen"),
        ),
        err,
      );
    } finally {
  ...
  const commitLabel = t(
    "topos.page.photo_intake.commit",
    "{count} Items übernehmen",
  ).replace("{count}", String(committableRows.length));
      ...
      <main className={`${pageMainNarrow} ${text}`}>
        ...
        <section className={`${card} p-4 mb-6 flex flex-col gap-3`}>
          <label className="flex flex-col gap-1">
            <span>
              {t(
                "topos.page.photo_intake.select_container",
                "In welchen Container?",
              )}
            </span>
            <select
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              {recognizing
                ? t(
                    "topos.page.photo_intake.recognizing",
...
function StagingRowCard({
  ...
  const unknownAiPath =
    row.categoryPath !== "" && !knownPaths.has(row.categoryPath);
  const badge =
    row.confidence === null ? null : confidenceBadge(row.confidence, t);
    ...
    <li data-testid={testId} className={`${card} p-3 flex flex-col gap-2`}>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <input
          ...
          placeholder={t(
            "topos.page.photo_intake.item_description",
            "Beschreibung",
          )}
