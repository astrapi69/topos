...
describe("Actions", () => {
  ...
  it("renders an open action with complete/edit/delete controls and an item link", async () => {
    ...
    expect(screen.getByTestId("action-item-link-5")).toHaveAttribute(
      "href",
      "/containers/1",
    );
  });
  ...
  it("shows a reopen control for a done action", async () => {
    const { api } = await import("../api/client");
    vi.mocked(api.actions.list).mockResolvedValue([
      { ...openAction, status: "done" },
    ]);
    renderActions();
    await waitFor(() => {
  ...
  it("creates an action through the form", async () => {
    ...
    fireEvent.change(screen.getByTestId("action-create-item"), {
      target: { value: "7" },
    });
    fireEvent.change(screen.getByTestId("action-create-text"), {
      target: { value: "Neue Aufgabe" },
    });
    fireEvent.change(screen.getByTestId("action-create-due"), {
