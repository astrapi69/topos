...
vi.mock("../ai", () => ({
  ...
  TOPOS_REGISTRY: {
    find: (id: string) => ({ id, label: "Anthropic (Claude)" }),
  },
...
async function recognize() {
  ...
  await waitFor(() =>
    expect(screen.getByTestId("photo-intake-row-0")).toBeInTheDocument(),
  );
}
...
describe("PhotoIntake", () => {
  ...
  it("shows the privacy notice before the first recognition", async () => {
    ...
    const [, message] = confirmMock.mock.calls[0] as unknown as [
      string,
      string,
    ];
  ...
  it("recognizes and fills the staging list with editable rows", async () => {
    ...
    expect(
      screen.getByTestId("photo-intake-row-0-confidence").textContent,
    ).toContain("90%");
    expect(
      screen.getByTestId("photo-intake-row-1-confidence").textContent,
    ).toContain("40%");
  ...
  it("commits selected rows via bulkCreate and navigates on full success", async () => {
    ...
    const payload = (api.items.bulkCreate as ReturnType<typeof vi.fn>).mock
      .calls[0][0];
    ...
    await waitFor(() =>
      expect(navigateMock).toHaveBeenCalledWith("/containers/42"),
    );
