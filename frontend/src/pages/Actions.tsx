...
export default function Actions() {
  ...
  async function handleComplete(id: number) {
    try {
      ...
      notify.success(
        t("topos.toast.action_done", "Aktion als erledigt markiert"),
      );
    } catch (e) {
      notify.error(
        errorMessage(
          e,
          t(
            "topos.toast.action_done_failed",
  ...
  async function handleReopen(id: number) {
    try {
      ...
      await actions.refresh();
      notify.success(
        t("topos.toast.action_reopened", "Aktion wieder geöffnet"),
      );
    } catch (e) {
      notify.error(
        errorMessage(
          e,
          t(
            "topos.toast.action_reopen_failed",
            "Aktion konnte nicht wieder geöffnet werden",
          ),
        ),
        e,
      );
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (newItemId === null) {
      notify.warning(
        t("topos.page.actions.item_required", "Eintrag ist erforderlich."),
      );
      return;
    }
    if (!newText.trim()) {
      notify.warning(
        t("topos.page.actions.text_required", "Text ist erforderlich."),
      );
      return;
    }
    try {
      ...
      notify.error(
        errorMessage(
          err,
          t(
            "topos.toast.action_create_failed",
    ...
    try {
      ...
      notify.error(
        errorMessage(
          e,
          t(
            "topos.toast.action_update_failed",
  ...
  async function handleDelete(action: ActionRow) {
    const ok = await confirm(
      ...
      t(
        "topos.confirm.delete_action_message",
        'Die Aktion "{text}" wird dauerhaft gelöscht.',
    ...
    try {
      ...
      notify.error(
        errorMessage(
          e,
          t(
            "topos.toast.action_delete_failed",
      ...
      <main className="p-4 sm:p-6">
        ...
        >
          <h1 data-testid="actions-title">
            {t("topos.page.actions.title", "Aktionen")}
          </h1>
          <button
            type="button"
        ...
        {creating && (
          ...
          >
            ...
            <FormField
              label={t("topos.page.actions.due_date", "Fälligkeitsdatum")}
            >
              <input
                type="date"
                className={input}
            ...
            <div style={{ display: "flex", gap: "0.5rem" }}>
              ...
              <button
                type="button"
                className={btn}
                data-testid="action-create-cancel"
                onClick={resetCreate}
              >
                      ...
                      {isEditing ? (
                        ...
                        >
                          <div>
                            ...
                            >
                              {t(
                                `topos.action.status.${action.status}`,
                                action.status,
                              )}
                            </span>
                            <br />
