...
describe("featureConfig registry", () => {
    it("activates backend-required features when a backend is reachable", () => {
        expect(featureRegistry.getState(FEATURES.EXCEL_IMPORT, ONLINE)).toBe("active");
        expect(featureRegistry.getState(FEATURES.CATEGORY_EDIT, ONLINE)).toBe("active");
    });

    it("disables backend-required features with a reason when offline", () => {
        expect(featureRegistry.getState(FEATURES.EXCEL_IMPORT, OFFLINE)).toBe("disabled");
        expect(featureRegistry.getState(FEATURES.CATEGORY_EDIT, OFFLINE)).toBe("disabled");
