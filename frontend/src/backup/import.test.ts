...
import {BackupValidationError, type ToposBackup, type ToposBackupData} from "./types";
...
describe("validateBackup", () => {
    ...
    it("rejects a foreign format", () => {
        expect(() => validateBackup({format: "other", version: 1, data: {}})).toThrow(
...
describe("importToposData (dexie path)", () => {
    it("replace clears existing data then inserts the backup", async () => {
        ...
        const result = await importToposData(
            backupOf({containers: [CONTAINER], items: [ITEM], categories: [CATEGORY], actions: [ACTION]}),
            "replace",
        );
    ...
    it("merge dedups items by (containerId, content)", async () => {
        ...
        const result = await importToposData(backupOf({containers: [CONTAINER], items: [ITEM]}), "merge");
    ...
    it("merge upserts a container by externalId without duplicating", async () => {
        ...
        const matches = (await db.containers.toArray()).filter((c) => c.externalId === 9001);
