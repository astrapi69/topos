...
export default function SearchResults({
  ...
  function subtitleFor(r: SearchResult): string {
    if (r.type === "item") {
      const label =
        r.containerId != null
          ? containerById.get(r.containerId)?.label
          : undefined;
      return [label, r.secondary].filter(Boolean).join(" · ");
    }
    if (r.type === "action") {
      const content =
        r.itemId != null ? itemById.get(r.itemId)?.content : undefined;
      const status = r.secondary
        ? t(`topos.action.status.${r.secondary}`, r.secondary)
        : "";
