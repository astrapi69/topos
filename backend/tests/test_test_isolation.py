...
(test suite dropped the user's production myapp.db) is the reason
this file exists.
"""
...
def test_test_mode_flag_is_set() -> None:
    """conftest.py must have set MYAPP_TEST=1 before any app import."""
    assert os.environ.get("MYAPP_TEST") == "1"
...
def test_database_url_respects_test_flag() -> None:
    ...
    assert "myapp.db" not in DATABASE_URL
