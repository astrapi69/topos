...
# PyInstaller spec for the Topos launcher (Windows, Linux, macOS).
#
# Build:  poetry run pyinstaller topos-launcher.spec
# Output:
#   Windows: dist/topos-launcher.exe (single-file, windowed, icon embedded)
#   Linux:   dist/topos-launcher (ELF binary)
#   macOS:   dist/Topos Launcher.app (bundle, icon if topos.icns exists)
...
# Read the target Topos version from backend/pyproject.toml and
...
_build_info = (
    _spec_dir / "topos_launcher" / "_build_info.py"
)
_build_info.write_text(
    "# AUTO-GENERATED at PyInstaller build time. Do not edit; regenerate "
    "via\n# `poetry run pyinstaller topos-launcher.spec`. See spec for "
    "source.\n"
    f'TOPOS_TARGET_VERSION = "{_target_version}"\n',
    encoding="utf-8",
)
...
exe = EXE(
    ...
    icon=["topos.ico"] if sys.platform == "win32" else None,
    version="version_info.txt" if sys.platform == "win32" else None,
)


# macOS .app bundle. Skipped on Windows and Linux.
# The icon file "topos.icns" is generated by the macOS CI workflow
...
if sys.platform == "darwin":
    app = BUNDLE(
        exe,
        name="Topos Launcher.app",
        icon="topos.icns" if os.path.exists("topos.icns") else None,
        bundle_identifier="com.topos.launcher",
        info_plist={
            "CFBundleName": "Topos Launcher",
            "CFBundleDisplayName": "Topos Launcher",
