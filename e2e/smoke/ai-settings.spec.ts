 ...
 *
 * Verifies the section renders against a live backend (it hides itself
 * when the AI endpoints are unreachable) and that the provider switch
 * drives the model/base-url controls client-side. Deliberately does NOT
 * click Save (would write the config overlay) or Test (would make a real
 * provider network call) - this stays a side-effect-free smoke.
 */
